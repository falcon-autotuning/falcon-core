...
```C++
using falcon_core::generic::FArraySP = typedef std::shared_ptr<FArray<T> >;
```
...
```C++
using falcon_core::generic::ListSP = typedef std::shared_ptr<List<Value> >;
```
...
```C++
using falcon_core::generic::MapSP = typedef std::shared_ptr<Map<Key, Value> >;
```
...
```C++
using falcon_core::generic::PairSP = typedef std::shared_ptr<Pair<T1, T2> >;
```
...
```C++
using falcon_core::generic::SongSP = typedef std::shared_ptr<Song>;
```
