...
TEST(PortTransformTest, InitializationAndApply) {
    auto identity_func = std::make_shared<falcon_core::math::analytic_functions::Identity>();
    auto constant_func = std::make_shared<falcon_core::math::analytic_functions::ConstantFunction>(42.0);

    falcon_core::instrument_interfaces::port_transforms::PortTransform identity_transform(identity_func);
    falcon_core::instrument_interfaces::port_transforms::PortTransform constant_transform(constant_func);
...
TEST(PortTransformsTest, ContainerBehavior) {
    auto identity_func = std::make_shared<falcon_core::math::analytic_functions::Identity>();
    auto constant_func = std::make_shared<falcon_core::math::analytic_functions::ConstantFunction>(3.14);

    auto identity_transform = std::make_shared<falcon_core::instrument_interfaces::port_transforms::PortTransform>(identity_func);
    auto constant_transform = std::make_shared<falcon_core::instrument_interfaces::port_transforms::PortTransform>(constant_func);

    falcon_core::instrument_interfaces::port_transforms::PortTransforms transforms;
...
TEST(PortTransformTest, SerializationRoundTrip) {
    auto constant_func = std::make_shared<falcon_core::math::analytic_functions::ConstantFunction>(123.456);
    falcon_core::instrument_interfaces::port_transforms::PortTransform original(constant_func);

    // Serialize to JSON
    std::string json = original.to_json_string();

    // Deserialize from JSON
    auto recreated = falcon_core::generic::Song::from_json_string<falcon_core::instrument_interfaces::port_transforms::PortTransform>(json);
...
TEST(PortTransformsTest, SerializationRoundTrip) {
    auto identity_func = std::make_shared<falcon_core::math::analytic_functions::Identity>();
    auto constant_func = std::make_shared<falcon_core::math::analytic_functions::ConstantFunction>(77.7);
    ...
    // Deserialize from JSON
    auto recreated = falcon_core::generic::Song::from_json_string<falcon_core::instrument_interfaces::port_transforms::PortTransforms>(json);
