...
from falcon_core.datatypes.connections import ScreeningGate
from falcon_core.datatypes.domains import KnobDomain
...
def test_knob_default_name():
    ...
    k = Knob(default_name="amplitude", units=CommonUnits.VOLT)
    assert k.default_name == "amplitude"
    assert k.units == CommonUnits.VOLT

    # Test with both name and default_name (name should take precedence)
    k2 = Knob(default_name="frequency", units=CommonUnits.HERTZ)
    assert k2.default_name == "frequency"
    assert k2.units == CommonUnits.HERTZ

    # Test equality with default_name
    k3 = Knob(default_name="amplitude", units=CommonUnits.VOLT)
    k4 = Knob(default_name="phase", units=CommonUnits.RADIAN)
    assert k == k3
    assert k != k4

    # Test with empty strings
    with pytest.raises(ValueError):
        Knob(default_name="", units=CommonUnits.VOLT)


def test_knob_serialization():
    """Test serialization of Knob class."""
    k = Knob(default_name="phase", units=CommonUnits.RADIAN, description="Signal phase")
...
def test_knobs_initialization():
    ...
    k1 = Knob(default_name="amp", units=CommonUnits.VOLT)
    k2 = Knob(default_name="freq", units=CommonUnits.HERTZ)
    k3 = Knob(default_name="phase", units=CommonUnits.RADIAN)
    ...
    # Test adding knobs
    k4 = Knob(default_name="offset", units=CommonUnits.VOLT)
    knobs.append(k4)
    assert len(knobs) == 4
...
def test_knobs_serialization():
    ...
    assert knobs_loaded["amp"].units == CommonUnits.VOLT
    assert knobs_loaded["freq"].units == CommonUnits.HERTZ
...
def test_knob_domain_initialization():
    ...
    domain = KnobDomain(default_name="voltage", bounds=bounds, units=CommonUnits.VOLT)
...
def test_knob_domain_serialization():
    ...
    k = Knob(
        default_name="power", units=CommonUnits.WATT, pseudo_name=ScreeningGate("S2")
    )
    ...
    assert domain_loaded.units == CommonUnits.WATT
