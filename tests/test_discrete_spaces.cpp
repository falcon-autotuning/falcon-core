...
TEST(DiscreteSpacesTest, CartesianDiscreteSpace2DSerializationRoundTrip) {
  ...
  std::string json = ds->to_json_string();
  auto        ds2 =
      CartesianDiscreteSpace2D::from_json_string<CartesianDiscreteSpace2D>(
          json);
...
// Complex, realistic projection test (mirroring Python
// test_get_projection_runs)
TEST(DiscreteSpacesTest, ComplexProjectionFunctionality) {
  ...
  auto                                          axes_discretizers =
      std::make_shared<Axes<BaseDiscretizer>>(discretizers);
  auto domain     = std::make_shared<Domain>(0.0, 1.0);
  ...
  auto                                            axes_coupled =
      std::make_shared<Axes<CoupledKnobDomain>>(coupled_domains);
