...
std::shared_ptr<Knob> make_clock_knob() {
  return std::make_shared<Knob>("CLOCK", nullptr, "CLOCK", nullptr, "Clock");
}
...
TEST(AnalyticFunctionTest, ValidatedConstantFunctionEvaluateAndSerialization) {
  ...
  for (const auto& name : stuff) {
    ports->push_back(
        std::make_shared<Knob>(name, nullptr, "PLUNGER", nullptr, ""));
  }
  ...
  auto        validated2 =
      ValidatedAF_KnobPorts::from_json_string<ValidatedAF_KnobPorts>(json);
...
class Mult2Function : public AnalyticFunction {
 public:
  ...
  double evaluate(double x) const override { return x * 2.0; }
