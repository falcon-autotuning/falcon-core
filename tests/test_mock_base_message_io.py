...
def mock_valkey():
    ...
    with patch(
        "falcon.communications.managers.locking_communication.valkey.Valkey"
    ) as mock_valkey_cls:
        ...
        # Re-configure default return values after reset
        mock_client.set.return_value = True
        mock_client.hset.return_value = True
        mock_client.eval.return_value = 1
...
def test_check_notifications(message_io, mock_valkey):
    """Test checking for notifications."""
    # Configure mock behavior for notifications
    mock_notification_data = Notifications(
        [Notification(channel="test_channel", timestamp=1234567890)]
    )

    # First no notifications
    mock_valkey.hget.return_value = None
    mock_valkey.hexists.return_value = False
    assert message_io.check_notifications() is None

    def hget_side_effect(*args, **kwargs):
        if "notifications" in str(args) and "test_app" in str(args):
            return mock_notification_data.to_json()
        return None

    # Then with notification
    mock_valkey.hget.side_effect = hget_side_effect
    mock_valkey.hexists.return_value = True

    # Mock Notification.from_json to return a proper Notification object
    # with patch(
    #     "falcon.communications.notification.Notification.from_json"
    # ) as mock_from_json:
    #     mock_notif = Mock(spec=Notification)
    #     mock_notif.channel = "test_channel"
    #     mock_from_json.return_value = mock_notif

    notification = message_io.check_notifications()
    assert notification == mock_notification_data.pop(0)
...
def test_command_workflow(message_io, mock_valkey):
    ...
    mock_valkey.hkeys.return_value = ["test_channel"]
    ...
    # Mock the necessary JSON encoding/decoding
    with patch("json.dumps") as mock_dumps:
        mock_dumps.return_value = '{"data": "test command"}'
...
def test_concurrent_notification_handling():
    """Test handling concurrent notification access with proper locking."""
    with patch(
        "falcon.communications.managers.locking_communication.valkey.Valkey"
    ) as mock_valkey_cls:
        # Create a thread-safe mock valkey client
        mock_client = MagicMock()
        ...
        def hkeys_side_effect(*args, **kwargs):
            ...
            return [f"channel_{i}" for i in range(3)]
            ...
            if "notifications" in str(args) and "test_app" in str(args):
                with notifications_lock:
                    if len(notifications_container) > 0:
                        return notifications_container.to_json()
            return None
        ...
        def hset_side_effect(*args, **kwargs):
            ...
            if (
                "notifications" in str(args)
                and "test_app" in str(args)
                and len(args) > 2
            ):
                with notifications_lock:
                    try:
        ...
        mock_client.hkeys.side_effect = hkeys_side_effect
        mock_client.hexists.side_effect = hexists_side_effect
        mock_client.hget.side_effect = hget_side_effect
        mock_client.hset.side_effect = hset_side_effect
        mock_client.hdel.side_effect = hdel_side_effect
        mock_client.set.side_effect = set_side_effect
        mock_client.eval.side_effect = eval_side_effect
        ...
        def check_notification():
            ...
            try:
                ...
                if result is not None:
                    ...
                    with call_lock:
                        call_order.append(
                            f"thread_success:{thread_id}:{result.channel}"
                        )
                else:
                    with call_lock:
        ...
        assert any("lock_acquire:" in call for call in call_order), (
            "No locks were acquired"
        )
        assert any("lock_release:" in call for call in call_order), (
            "No locks were released"
        )
...
def test_collision_during_channel_acquisition(message_io, mock_valkey):
    ...
    mock_valkey.hkeys.return_value = ["channel1"]
...
def test_message_collision_with_concurrent_threads():
    ...
    with patch(
        ...
        def hkeys_side_effect(*args, **kwargs):
            with call_lock:
                call_order.append(f"hkeys:{args}")
            return list(channel_states.keys())
        ...
        def hget_side_effect(*args, **kwargs):
            ...
            if "channel_status" in str(args):
                channel_key = args[1]
                return channel_states.get(channel_key, ChannelStatusType.IDLE.value)
...
def test_wait_for_notification_timeout(message_io, mock_valkey):
    """Test timeout when waiting for notification."""
    # No notifications available
    mock_valkey.hget.return_value = None
    # Should time out
    with pytest.raises(TimeoutError):
        message_io.wait_for_notification()
