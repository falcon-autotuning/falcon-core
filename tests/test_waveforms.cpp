...
TEST(WaveformTest, SerializationRoundTrip) {
    ...
    EXPECT_DOUBLE_EQ(
        dynamic_cast<ConstantTransform*>(recreated->get_transforms()[1].get())->apply(0.0),
        42.0
    );
}
...
TEST(CartesianWaveformTest, NDConstructionAndSerialization) {
    ...
    // ND: 3 axes
    auto divisions = std::make_shared<falcon_core::math::Axes<int>>(std::vector<int>{10, 20, 30});
    auto axes = std::make_shared<falcon_core::math::Axes<falcon_core::math::domains::CoupledKnobDomain>>();
    auto increasing = std::make_shared<falcon_core::math::Axes<std::map<std::string, bool>>>();

    auto identity_transform = std::make_shared<IdentityTransform>();
    auto constant_transform = std::make_shared<ConstantTransform>(1.23);

    auto waveform = CartesianWaveform::from_divisions(
        divisions, axes, increasing, {identity_transform, constant_transform}, domain
    );
    ...
    auto recreated = falcon_core::generic::Song::from_json_string<CartesianWaveform>(json);
    ASSERT_EQ(recreated->get_transforms().size(), 2);
}
...
TEST(CartesianWaveform1DTest, ConstructionAndSerialization) {
    ...
    auto shared_domain = std::make_shared<falcon_core::math::domains::CoupledKnobDomain>();
    auto increasing = std::make_shared<falcon_core::math::Axes<std::map<std::string, bool>>>();
    ...
    // Serialization round-trip
    std::string json = waveform->to_json_string();
    auto recreated = falcon_core::generic::Song::from_json_string<CartesianWaveform1D>(json);
...
TEST(CartesianWaveform2DTest, ConstructionAndSerialization) {
    ...
    auto divisions = std::make_shared<falcon_core::math::Axes<int>>(std::vector<int>{5, 5});
    auto axes = std::make_shared<falcon_core::math::Axes<falcon_core::math::domains::CoupledKnobDomain>>();
    auto increasing = std::make_shared<falcon_core::math::Axes<std::map<std::string, bool>>>();
    ...
    // Serialization round-trip
    std::string json = waveform->to_json_string();
    auto recreated = falcon_core::generic::Song::from_json_string<CartesianWaveform2D>(json);
    ASSERT_EQ(recreated->get_transforms().size(), 1);
}
