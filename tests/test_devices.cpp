...
//   "falcon_core.physics.device_structures.channel"); ASSERT_EQ
//   (j["__class__"], "Channel"); ASSERT_EQ (j["_name"], 1);
// }
//
...
//   ASSERT_THROW (connections.index (std::make_shared<Gate> ("g3")),
//                 std::out_of_range);
// }
//
...
//   auto quantity    = std::make_shared<Quantity<std::complex<double>>> (
//       std::complex<double> (50.0, 25.0), symbol_unit);
...
//   auto symbol_unit = std::make_shared<SymbolUnit> (unit, "Ohm");
//   auto quantity    = std::make_shared<Quantity<std::complex<double>>> (
//       std::complex<double> (50.0, 0.0), symbol_unit);
//   auto imp1 = std::make_shared<Impedance> (quantity);
//
//   Impedances impedances;
//   impedances.append (imp1);
//   // A bit hard to test more without an accessor, but we can check
//   // serialization
...
//   auto   ohmics = std::make_shared<BaseConnections<Ohmic>> ();
//   auto   gates  = std::make_shared<BaseConnections<Gate>> ();
//   auto   scc    = std::make_shared<StandardConfigConnections> (ohmics,
//   gates); Loader loader (scc);
//
...
//   ASSERT_THROW (channels.index (std::make_shared<Channel> (3)),
//                 std::out_of_range);
// }
