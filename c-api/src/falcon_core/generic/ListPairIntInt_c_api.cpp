...
ListPairIntIntHandle ListPairIntInt_fill_value(size_t count, PairIntIntHandle value) {
    ...
    auto stored_obj = *static_cast<falcon_core::generic::Pair<int, int>SP*>(value);
...
throw std::invalid_argument("Null data handle passed to ListPairIntInt_create");
    ...
    for (size_t i = 0; i < count; ++i) {
        vec.push_back(*static_cast<falcon_core::generic::Pair<int, int>SP*>(data[i])); 
    }
...
for (size_t i = 0; i < n; ++i) {
    out_buffer[i] = new falcon_core::generic::Pair<int, int>SP(list->items()[i]);
}
    return n;
    FALCON_C_API_END(0)
}
