...
class Is1D : public virtual generic::IFArray<T> {
 public:
   ...
   */
  inline T get_distance() const { return std::abs(get_start() - get_end()); }
  /**
   * @brief Get the mean of the 1D array.
   * @returns The mean value.
   */
  inline double get_mean() const { return xt::mean(this->data())(); }

  /**
   * @brief Get the standard deviation of the 1D array.
   * @returns The standard deviation value.
   */
  inline double get_std() const { return xt::stddev(this->data())(); }
   ...
   */
  inline size_t get_closest_index(double value) const {
    auto& arr = this->data();
    auto  it =
   ...
   */
  inline generic::ListSP<generic::FArray<T>> even_divisions(
