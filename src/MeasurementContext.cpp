...
nlohmann::json MeasurementContext::to_json() const {
    nlohmann::json j;
    add_metadata(j, "falcon_core.autotuner_interfaces.contexts.measurement_context", "MeasurementContext");
    j["_connection"] = _connection->to_json();
    j["_unit"] = _unit->to_json();
    return j;
}

size_t MeasurementContext::hash() const {
