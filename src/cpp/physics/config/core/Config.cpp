...
auto double_equal = [](double a, double b) {
...
Config::Config(const Config &other) : StandardConfigConnections(other) {
  ...
  std::unique_lock<std::shared_timed_mutex> lock_max_bound(_mu_max_bound,
                                                           std::defer_lock);
  std::lock(lock_channels, lock_groups, lock_wiring_DC,
            lock_num_unique_channels, lock_adjacency, lock_max_safe_diff,
            lock_min_bound, lock_max_bound);
  if (!other.groups() || !other.wiring_DC() || !other.adjacency() ||
      !other.channels()) {
    throw std::invalid_argument(
        "Config: The adjacency, wiring, groups, and channels are not "
  ...
  _max_safe_diff = other.max_safe_diff();
  _min_bound = other.min_bound();
  _max_bound = other.max_bound();
}
...
Config::Config(
    ...
    const AdjacencySP &adjacency, double max_safe_diff, double min_bound,
    double max_bound)
    : StandardConfigConnections(screening_gates, reservoir_gates, plunger_gates,
                                barrier_gates, ohmics),
      _wiring_DC(wiring_DC),
      _channels(std::make_shared<autotuner_interfaces::names::Channels>()),
      _groups(groups), _adjacency(adjacency), _max_safe_diff(max_safe_diff),
      _min_bound(min_bound), _max_bound(max_bound) {
  if (!adjacency || !wiring_DC || !groups) {
    throw std::invalid_argument(
        "Config: The adjacency, wiring, and groups are not permitted to be "
        "null.");
  }
...
void Config::check_group_consistency() const {
  ...
  for (const GroupSP &group : *all_groups) {
    const std::vector<device_structures::Connections> group_connections =
        std::vector<device_structures::Connections>{
            *group->screening_gates(), *group->reservoir_gates(),
            *group->plunger_gates(), *group->barrier_gates(), *group->ohmics()};
...
device_structures::ImpedanceSP
Config::get_impedance(const device_structures::ConnectionSP &connection) const {
  if (!connection) {
...
GroupSP
Config::select_group(const autotuner_interfaces::names::GnameSP &gname) const {
  if (has_gname(gname)) {
...
device_structures::ConnectionSP Config::get_associated_ohmic(
  ...
  auto all_groups = *get_all_groups();
  for (const GroupSP &group : all_groups) {
    if (!group->has_gate(reservoir_gate))
      continue;
...
autotuner_interfaces::names::GnameSP
Config::get_gname(const autotuner_interfaces::names::ChannelSP &channel) const {
  if (!has_channel(channel))
...
bool Config::ohmic_in_channel(
  ...
  if (!has_channel(channel) || !ohmic->is_ohmic())
    return false;
...
device_structures::ConnectionsSP Config::get_shared_channel_barrier_gates(
  ...
  for (const device_structures::ConnectionSP &bgate : *channel_gates) {
    if (shared_bgates->contains(bgate))
      outs->push_back(bgate);
  }
  return outs;
}
device_structures::ConnectionsSP Config::get_shared_channel_plunger_gates(
  ...
  for (const device_structures::ConnectionSP &pgate : *channel_gates) {
    if (shared_bgates->contains(pgate))
      outs->push_back(pgate);
  }
  return outs;
}
device_structures::ConnectionsSP Config::get_shared_channel_reservoir_gates(
  ...
  for (const device_structures::ConnectionSP &rgate : *channel_gates) {
    if (shared_bgates->contains(rgate))
      outs->push_back(rgate);
  }
  return outs;
}
device_structures::ConnectionsSP Config::get_shared_channel_screening_gates(
  ...
  for (const device_structures::ConnectionSP &sgate : *channel_gates) {
    if (shared_gates->contains(sgate))
      outs->push_back(sgate);
  }
  return outs;
}
device_structures::ConnectionsSP Config::get_shared_channel_dot_gates(
  ...
  for (const device_structures::ConnectionSP &dgate : *channel_gates) {
    if (shared_gates->contains(dgate))
      outs->push_back(dgate);
  }
  return outs;
}
...
bool Config::operator==(const Config &other) const {
  ...
  return StandardConfigConnections::operator==(other) &&
         ...
         double_equal(_max_safe_diff, other._max_safe_diff) &&
         double_equal(_min_bound, other._min_bound) &&
         double_equal(_max_bound, other._max_bound);
}
