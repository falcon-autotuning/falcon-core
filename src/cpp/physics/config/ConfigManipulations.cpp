#include "falcon-core/physics/config/ConfigManipulations.hpp"
...
core::ConfigSP
ConfigManipulations::unpack_device_config(const YAML::Node &config) const {
  ...
  for (const auto &group_pair : config["groups"]) {
    ...
    core::GroupSP new_group = std::make_shared<core::Group>(
        ...
        value["NumDots"].as<int>(), group_connections->screening_gates(),
        group_connections->reservoir_gates(),
        group_connections->plunger_gates(), group_connections->barrier_gates(),
        order);
  ...
  total_gates.insert(total_gates.end(), connections->screening_gates()->begin(),
                     connections->screening_gates()->end());
  total_gates.insert(total_gates.end(), connections->reservoir_gates()->begin(),
                     connections->reservoir_gates()->end());
  total_gates.insert(total_gates.end(), connections->plunger_gates()->begin(),
                     connections->plunger_gates()->end());
  total_gates.insert(total_gates.end(), connections->barrier_gates()->begin(),
                     connections->barrier_gates()->end());
  auto adjacency = _extract_adjacency(
      config, std::make_shared<device_structures::Connections>(total_gates));

  return std::make_shared<core::Config>(
      connections->screening_gates(), connections->plunger_gates(), ohmics,
      connections->barrier_gates(), connections->reservoir_gates(),
      std::make_shared<
          generic::Map<autotuner_interfaces::names::Gname, core::Group>>(
          groups),
      wiring_DC, adjacency, config["max_safe_diff"].as<double>(),
      config["safe_voltage_bounds"][0].as<double>(),
      config["safe_voltage_bounds"][1].as<double>());
}
...
core::AdjacencySP ConfigManipulations::_extract_adjacency(
  ...
  for (size_t i = 0; i < total_gate_names.size(); ++i) {
    ...
    for (size_t j = 0; j < total_gate_names.size(); ++j) {
      if (std::find(gate_names.begin(), gate_names.end(),
...
device_structures::ImpedancesSP ConfigManipulations::_extract_dcwiring(
    const YAML::Node &map, const device_structures::ConnectionsSP &ohmics,
  ...
  for (const auto &entry : impedances) {
    ...
    outs.push_back(std::make_shared<device_structures::Impedance>(
        gt, values["resistance"].as<double>(),
...
ConfigManipulations::_extract_barrier_gates(const std::string raw) const {
...
device_structures::ConnectionsSP
ConfigManipulations::_extract_plunger_gates(const std::string raw) const {
  auto gate_names = split_on_semicolon(raw);
...
device_structures::ConnectionsSP
ConfigManipulations::_extract_ohmics(const std::string raw) const {
  auto gate_names = split_on_semicolon(raw);
...
ConfigManipulations::_extract_standard_group_config_connections(
  ...
  return std::make_shared<core::StandardConfigConnections>(
      screening_gates, reservoir_gates, plunger_gates, barrier_gates,
