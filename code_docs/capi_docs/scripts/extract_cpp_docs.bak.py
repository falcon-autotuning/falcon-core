...
(function/class/struct/using/enum). Emits either:
  ...
  1) Per-file metadata (default): one YAML (or JSON fallback) file per header,
     saved to a *mirrored* tree where "cpp" is replaced by "cpp_metadata".
     Example:
         /path/.../cpp/include/.../Thing.hpp
     ->  /path/.../cpp_metadata/include/.../Thing_metadata.yml
...
Usage examples:
  # Process one header (per-file mirrored outputs)
...
#     # 1) Namespace hint (unchanged)
#     ns_parts = [m.group(1) for m in NS_OPEN_RE.finditer(text)]
#     ns_hint = '::'.join(ns_parts) if ns_parts else ''

#     # 2) Find class scopes (start/end ranges where methods live)
#     class_scopes = []

#     def find_class_end(start_pos: int) -> int:
#         """
#         Given the position of the '{' after 'class Name', scan forward to find
#         the matching '}' using a simple brace counter.
#         """
#         depth = 0
#         for i, ch in enumerate(text[start_pos:], start=start_pos):
#             if ch == '{':
...
#     def containing_class(pos: int) -> Optional[str]:
#         for cs in class_scopes:
#             if cs["start"] <= pos < cs["end"]:
#                 return cs["name"]
#         return None

#     # 3) Walk all doc-comment+decl pairs
#     for m in DECL_RE.finditer(text):
#         comment = m.group('comment') or ''
#         decl = (m.group('decl') or '').strip()
#         info = likely_kind_and_name(decl)
#         base_name = info['name']  # as detected by FUN_NAME_RE, etc.
#         kind = info['kind']

#         # Where in the file this declaration starts
#         decl_start = m.start('decl')
#         cls_name = containing_class(decl_start)

#         full_name = base_name

#         if base_name:
#             # If this is inside a class, and not already scoped, and not a ctor
#             # (ctor has same name as the class), prefix with Class::method.
#             if cls_name and '::' not in base_name and base_name != cls_name:
#                 full_name = f"{cls_name}::{base_name}"

#             # Now add namespace if not already present
#             if ns_hint and not full_name.startswith(ns_hint + '::'):
#                 full_name = f"{ns_hint}::{full_name}"
...
def _mirror_path_for_header(header_path: Path, same_dir: bool = False) -> Path:
    """
    /.../cpp/include/foo/bar/Baz.hpp ->
    /.../cpp_metadata/include/foo/bar/Baz_metadata.yml  (default)
    or next to header with --same-dir.
    """
    header_path = header_path.resolve()
    stem = header_path.stem + "_metadata"
    out_name = stem + ".yml"
    if same_dir:
        return header_path.with_name(out_name)

    parts = list(header_path.parts)
    try:
        idx = parts.index("cpp")
        parts[idx] = "cpp_metadata"
    except ValueError:
        return header_path.with_name(out_name)

    return Path(*parts).with_name(out_name)
...
def write_per_file_outputs(
    ...
    for file_str, units in results.items():
        hdr = Path(file_str)
        out_path = _mirror_path_for_header(hdr, same_dir=same_dir)
        out_path.parent.mkdir(parents=True, exist_ok=True)
        if want_yaml:
            with open(out_path, "w", encoding="utf-8") as fh:
                yaml.safe_dump(units, fh, sort_keys=False, allow_unicode=True)  # type: ignore
        else:
...
def main() -> None:
    ...
    ap.add_argument('--out', help='Aggregate output file (.json or .yml/.yaml). If omitted, writes one metadata file per header.')
    ap.add_argument('--same-dir', action='store_true', help='Write the per-header metadata file next to the header (instead of mirrored cpp_metadata tree).')
    ap.add_argument('--exts', default='.h,.hpp,.hh,.hxx', help='Comma-separated extensions to scan (default: .h,.hpp,.hh,.hxx)')
    ap.add_argument('--no-recursive', action='store_true', help='Do not recurse into subdirectories when inputs are directories.')
    ...
    data = walk_and_extract(paths, exts=tuple(exts), recursive=recursive,
                            include_rx=include_rx, exclude_rx=exclude_rx)
    ...
    # - Else: per-file outputs (mirrored tree by default, or same dir with --same-dir)
    if args.out:
        out = Path(args.out)
        ...
        write_per_file_outputs(data, same_dir=args.same_dir)
