...
def main() -> None:
    ...
    def base_name(fq_name: str) -> str:
        """Extract the simple function name from a fully qualified name."""
        return fq_name.split("::")[-1]


    # 1) Build a representative fully-qualified name for each doc’d function name
    doc_fqn_by_name: Dict[str, str] = {}
    for fq in cpp_doc_from_headers:
        name = base_name(fq)
        if name in cpp_names_doc and name not in doc_fqn_by_name:
            doc_fqn_by_name[name] = fq

    # 2) Which names are already ported?
    ported_names: Set[str] = {base_name(fq) for fq in ported_from_headers}

    # 3) Names left to port, according to the regex-doc universe (cpp_doc)
    left_names: Set[str] = cpp_names_doc - ported_names

    # 4) Turn those names back into fully-qualified functions for the log
    left_to_port_full: Set[str] = {
        doc_fqn_by_name[name]
        for name in left_names
        if name in doc_fqn_by_name
    }
    ...
    # Make log directory in working directory if it doesn't exist
    ...
    write_list(
        log_dir / "cpp_scanned_doc_functions.log",
    ...
    write_list(
        log_dir / "cpp_metadata_doc_functions.log",
        cpp_doc_meta,
    ...
    write_list(
        log_dir / "cpp_ported_functions.log",
        cpp_ported,
    ...
    write_list(
        log_dir / "cpp_left_to_port_functions.log",
        left_to_port_full,
