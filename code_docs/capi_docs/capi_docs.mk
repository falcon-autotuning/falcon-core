...
# --------------------------------------------------------------------
DOC_CODE_ROOT 		  := ./code_docs/capi_docs

CPP_ROOT              := ./cpp
C_API_ROOT            := ./c-api
C_API_ARCHIVE_ROOT    := $(DOC_CODE_ROOT)/archive
C_API_PRISTINE        := $(C_API_ARCHIVE_ROOT)/c-api_no_docs

CPP_INCLUDE_ROOT      := $(CPP_ROOT)/include
C_API_INCLUDE_ROOT    := $(C_API_ROOT)/include

CPP_METADATA_ROOT     := $(DOC_CODE_ROOT)/cpp_metadata #TODO: update extraction script to use this path
# CPP_METADATA_ROOT     := ./cpp_metadata

AUTO_MAPS_DIR         := $(C_API_INCLUDE_ROOT)/falcon_core
USER_MAPS_DIR         := $(DOC_CODE_ROOT)/c-api_user_maps

LOGS_DIR              := $(DOC_CODE_ROOT)/logs

# Python scripts (adjust these names if yours differ)
PYTHON_SCRIPTS		  := ./scripts
EXTRACT_CPP_DOCS      := $(DOC_CODE_ROOT)/$(PYTHON_SCRIPTS)/extract_cpp_docs.py
GENERATE_C_API_MAPS   := $(DOC_CODE_ROOT)/$(PYTHON_SCRIPTS)/generate_c_api_maps.py
INJECT_C_DOCS         := $(DOC_CODE_ROOT)/$(PYTHON_SCRIPTS)/inject_c_docs.py
COVERAGE_SCRIPT       := $(DOC_CODE_ROOT)/$(PYTHON_SCRIPTS)/doxygen_port_coverage.py
