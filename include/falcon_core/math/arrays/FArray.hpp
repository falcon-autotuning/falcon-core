...
public:
    ...
    auto operator()(Args&&... args) -> decltype(_data(std::forward<Args>(args)...)) {
        return _data(std::forward<Args>(args)...);
    }
    template <typename... Args>
    auto operator()(Args&&... args) const -> decltype(_data(std::forward<Args>(args)...)) {
        return _data(std::forward<Args>(args)...);
    }
    ...
    FArray& operator+=(const FArray& other) { _data += other._data; return *this; }
    FArray& operator-=(const FArray& other) { _data -= other._data; return *this; }
    FArray& operator*=(const FArray& other) { _data *= other._data; return *this; }
    FArray& operator/=(const FArray& other) { _data /= other._data; return *this; }
    ...
    FArray& operator=(const array_type& arr) { _data = arr; return *this; }
