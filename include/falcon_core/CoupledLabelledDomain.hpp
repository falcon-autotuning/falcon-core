...
public:
    CoupledLabelledDomain(double min_val, double max_val, std::shared_ptr<T> label, std::vector<std::shared_ptr<LabelledDomain<T>>> coupled_domains)
        : LabelledDomain<T>(min_val, max_val, label), _coupled_domains(std::move(coupled_domains)) {}

    const std::vector<std::shared_ptr<LabelledDomain<T>>>& coupled_domains() const {
        return _coupled_domains;
    }

    nlohmann::json to_json() const override {
        nlohmann::json j = LabelledDomain<T>::to_json();
        add_metadata(j, "falcon_core.math.domains.coupled_labelled_domain", "CoupledLabelledDomain");
