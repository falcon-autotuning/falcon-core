...
template<typename T>
class Quantity : public Jsonable {
public:
    Quantity(T value, std::shared_ptr<SymbolUnit> unit) : _value(value), _unit(std::move(unit)) {}
...
template<typename T>
nlohmann::json Quantity<T>::to_json() const {
    nlohmann::json j;
