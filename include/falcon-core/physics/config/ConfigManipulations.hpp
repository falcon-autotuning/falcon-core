...
public:
   ...
   */
  core::AdjacencySP
  _extract_adjacency(const YAML::Node &map,
   ...
   */
  device_structures::ImpedancesSP
  _extract_dcwiring(const YAML::Node &map,
  ...
  _extract_standard_config_connections(const YAML::Node &config) const;
  /**
   * @brief Extracts the standard connections from a config dictonary
   * @param config The dictionary with keys that are gate types and values which
   ...
   */
  core::StandardConfigConnectionsSP
  _extract_standard_group_config_connections(const YAML::Node &config) const;
  /**
   ...
   */
  device_structures::ConnectionsSP
  _extract_order(const std::string raw,
                 const core::StandardConfigConnectionsSP &connections) const;
};
} // namespace config
} // namespace physics
